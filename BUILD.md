...
# 另一个非显而易见的坑：Termux 会被系统冻结

## 现象

构建或 `git push` 这类命令**卡住不返回，直到工具超时**（120s / 300s），
但**同样的命令写成脚本放到后台跑就一切正常**。

关键线索：连 `echo ALIVE`、`git remote -v` 这种纯本地、不碰网络的命令也超时。
**所以这不是网络问题。**

## 原因

Termux 退到后台一段时间后会被 Android **冻结进程**（cached app freezer）。
冻结期间它的命令队列不再推进，正在执行的命令也原地停住。
系统内存压力会显著加速这个冻结——实测出问题时 `load average 9.35`（5 核设备）。

从外部观察到的表现就是「命令永远不返回」。

## 排查方法

用任意另一个 shell（例如 Shizuku）看系统状态：

```bash
uptime                                    # load average 高得离谱
free -m                                   # used 接近上限
ps -A -o PID,RSS,NAME --sort=-RSS | head  # 没有失控进程 → 是冻结，不是内存泄漏
```

## 解法

1. **先把 Termux 切到前台唤醒它**，被冻结的进程会立刻恢复：

```bash
am start -n com.termux/.HomeActivity   # 或用启动器手动点开
```

2. **长任务放后台 + 轮询日志**，不要用一个前台命令死等：

```bash
nohup bash long_task.sh > /dev/null 2>&1 &
tail -5 long_task.log     # 每次只看日志，命令本身立刻返回
```
...
| 方式 | 结果 |
|---|---|
| 前台执行 `gh repo create --push` | 卡死直至 300 秒超时，`origin` 都没配上 |
| 同样的操作写成脚本后台执行 | 建仓库 + 推 main + 推 tag + 传 APK，**全程约 2 分钟完成** |

**结论：遇到莫名其妙的超时，先检查 Termux 是否被冻结，而不是先怀疑网速。**
